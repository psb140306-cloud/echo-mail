  ...
  try {
    ...
    if (rulesData.success) {
      ...
      rulesData.data.forEach((rule, index) => {
        console.log(
          `   ${index + 1}. ${rule.region}: 오전 ${rule.morningCutoff}(${rule.morningDeliveryDays}일), 오후 ${rule.afternoonCutoff}(${rule.afternoonDeliveryDays}일)`
        )
      })
    } else {
    ...
    if (createdRuleId) {
      ...
      if (ruleDetailData.success) {
        ...
        console.log(
          `   - 오전 마감: ${ruleDetailData.data.morningCutoff} (${ruleDetailData.data.morningDeliveryDays}일 후 배송)`
        )
        console.log(
          `   - 오후 마감: ${ruleDetailData.data.afternoonCutoff} (${ruleDetailData.data.afternoonDeliveryDays}일 후 배송)`
        )
      } else {
    ...
    if (holidaysData.success) {
      ...
      holidaysData.data.forEach((holiday, index) => {
        ...
        console.log(
          `   ${index + 1}. ${date} - ${holiday.name} ${holiday.isRecurring ? '(매년 반복)' : ''}`
        )
      })
    } else {
    ...
    // 3-3. 다음 영업일 조회
    console.log('\n3-3. 다음 영업일 조회')
    const nextBusinessDayResponse = await fetch(
      `${API_BASE}/delivery/calculate?action=next-business-day&date=2025-01-15`
    )
    ...
    if (nextBusinessDayData.success) {
      ...
      console.log(
        `   - 기준일: ${new Date(nextBusinessDayData.data.baseDate).toLocaleDateString('ko-KR')}`
      )
    ...
    // 3-4. 영업일 간격 계산
    console.log('\n3-4. 영업일 간격 계산')
    const businessDaysResponse = await fetch(
      `${API_BASE}/delivery/calculate?action=business-days-between&startDate=2025-01-15&endDate=2025-01-25`
    )
    ...
    if (businessDaysData.success) {
      ...
      console.log(
        `   - 시작일: ${new Date(businessDaysData.data.startDate).toLocaleDateString('ko-KR')}`
      )
      console.log(
        `   - 종료일: ${new Date(businessDaysData.data.endDate).toLocaleDateString('ko-KR')}`
      )
