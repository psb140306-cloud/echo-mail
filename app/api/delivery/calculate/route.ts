...
import { calculateDeliveryDate, getNextBusinessDay, getBusinessDaysBetween } from '@/lib/utils/delivery-calculator'

// 납품일 계산 요청 스키마
const calculateDeliverySchema = z.object({
  region: z.string().min(1, '지역은 필수입니다'),
  orderDateTime: z.string().refine(
    (dateStr) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return createErrorResponse(error instanceof Error ? error.message : '납품일 계산에 실패했습니다.')
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      default:
        return createErrorResponse('지원하지 않는 액션입니다. (next-business-day, business-days-between)', 400)
    }

  } catch (error) {
    logger.error('영업일 계산 API 실패:', error)
    return createErrorResponse(error instanceof Error ? error.message : '영업일 계산에 실패했습니다.')
  }
}