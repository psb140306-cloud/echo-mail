...
- [ ] **데이터베이스 설계**
  - [ ] Announcement 테이블 생성 (공지 내용, 발송 채널, 상태, 예약 시간)
  - [ ] AnnouncementRecipient 테이블 생성 (수신자별 발송 상태)
  - [ ] 테넌트별 데이터 격리 적용

- [ ] **공지 발송 API**
  - [ ] POST /api/announcements (공지 생성)
  - [ ] GET /api/announcements (공지 목록 조회)
  - [ ] GET /api/announcements/:id (공지 상세 조회)
  - [ ] PUT /api/announcements/:id (공지 수정 - 발송 전만 가능)
  - [ ] DELETE /api/announcements/:id (공지 삭제/취소)
  - [ ] POST /api/announcements/:id/send (즉시 발송)
...
- [ ] **발송 기능**
  - [ ] 즉시 발송 기능
  - [ ] 예약 발송 기능 (날짜/시간 지정)
  - [ ] 예약 발송 스케줄러 (node-cron)
  - [ ] 대량 발송 큐 처리 (Rate limiting 적용)
  - [ ] 발송 실패 시 재시도 로직

- [ ] **공지 관리 UI** (`/announcements`)
  - [ ] 공지 목록 페이지
  - [ ] 공지 작성 페이지
    - [ ] 메시지 내용 입력 (변수 지원: {고객명}, {회사명} 등)
    - [ ] 발송 채널 선택 (SMS/카카오톡/둘 다)
    - [ ] 수신자 선택 UI (체크박스/필터)
    - [ ] 예약 발송 설정
    - [ ] 미리보기 기능
  - [ ] 공지 상세/수정 페이지
