...
##### 1. 특수문자 디코딩 강화 적용
- **커밋**: `b205cba` feat: 특수문자 디코딩 강화 적용
- **문제**: 스팸 메일 발신자 이름 `만족도∧1위@nru.com`이 `????????12@nru.com`로 표시
- **원인**: MIME 디코더가 특수문자 `∧`를 처리하지 못함
- **해결**:
  - `lib/mail/email-decoder.ts` 신규 생성
    - 여러 인코딩 전략 (MIME → Buffer → Fallback)
    - 특수문자 정규화 (`∧` → `^`, `∨` → `v` 등)
    - 깨진 문자 감지 (replacement character `�`, 연속 `???` 등)
  - `lib/mail/mail-monitor-service.ts` 수정
    - `decodeEmailHeader` import 추가
    - `decodeMimeHeader` 함수 강화 (특수문자 정규화 포함)
    - 발신자 이름 디코딩 로직 개선
- **효과**: 이제 `만족도∧1위`가 `만족도^1위`로 정확히 표시됨

#### 📋 다음 작업 계획 (스팸 및 IMAP 동기화)

##### Phase 7: 스팸 필터링 및 IMAP 동기화 (예정)

###### 2. 스팸 필터 추가 (예정)
- **모듈**: `lib/mail/spam-filter.ts` (이미 생성됨)
- **기능**:
  - 도메인 블랙리스트 체크 (nru.com 등)
  - 발신자 이름 패턴 검증 (연속 `????`, 과도한 특수문자 등)
  - 제목/본문 키워드 매칭 (성인/광고/금융 카테고리)
  - 점수 기반 스팸 판정 (0-100점, 60점 이상 스팸)
- **통합 위치**: `mail-monitor-service.ts` `processEmail()` 함수
- **참고 문서**: `SPAM_SYNC_INTEGRATION.md` 섹션 1
- **예상 효과**: 다음 메일 스팸 필터 처리 전 Echo Mail에서 사전 차단

###### 3. IMAP 동기화 추가 (예정)
- **모듈**: `lib/mail/imap-sync-service.ts` (이미 생성됨)
- **기능**:
  - DB 저장 메일과 IMAP INBOX UID 비교
  - 누락된 메일은 스팸함 확인
  - 스팸함 이동 시 상태 IGNORED로 변경
  - 완전 삭제 시에도 상태 IGNORED로 변경
- **Cron Job**: `app/api/cron/sync-imap/route.ts` (신규)
  - 매일 새벽 2시 실행
  - 최근 7일 메일만 동기화
  - 활성 테넌트별 순차 처리
- **참고 문서**: `SPAM_SYNC_INTEGRATION.md` 섹션 2
- **예상 효과**: 다음 메일에서 스팸 이동한 메일 자동 감지 및 상태 업데이트
...
- [ ] 스팸 필터 테스트
  ...
  - 과도한 특수문자 감지 테스트
- [ ] IMAP 동기화 테스트
  - 테스트 메일 발송 → Echo Mail 수신
  - 다음 메일함에서 스팸으로 이동
  - Cron Job 수동 실행 후 상태 확인

---

**마지막 업데이트**: 2025-12-24
**다음 작업 시작 지점**: 특수문자 디코딩 적용 완료. 남은 작업:
- Phase 6: 발송 비용 통계, CSV 내보내기, 발송 실패 재시도, 카카오 알림톡 연동
- Phase 7: 스팸 필터 통합, IMAP 동기화 Cron Job 추가 (계획 완료, 구현 예정)
